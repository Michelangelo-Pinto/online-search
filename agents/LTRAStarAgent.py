    ...
    def __call__(self, s1):  # as of now s1 is a state rather than a percept
        if self.problem.goal_test(s1):
            ...
            if s1 not in self.H:
                ...
                self.H[s1] = float('inf')
            #print(f"i am here: {self.s} and i think i am this far from goal: {self.H[self.s]}")
            ...
            if self.s is not None:
                ...
                self.H[self.s] = min(self.problem.agent_cost_evaluation_function(self.problem, self.s, b, self.problem.output(self.s, b), self.H)
                                     for b in self.problem.actions(self.s))

            # an action b in problem.actions(s1) that minimizes costs
            self.a = min(self.problem.actions(s1),
                         key=lambda b: self.problem.agent_cost_evaluation_function(self.problem, s1, b, self.problem.output(s1, b), self.H))

            #DEBUG#print(f"I chose to go in: {self.a()} and I think i will be this far from goal: {self.H[s1]}")
